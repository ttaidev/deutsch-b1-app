...
export function WordModal({ wordData, isOpen, onClose, isLoading = false }: WordModalProps) {
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-slate-900/60 backdrop-blur-xs animate-in fade-in">
      <div className="bg-white rounded-3xl max-w-md w-full p-6 shadow-2xl border border-slate-200 relative animate-in zoom-in-95 min-h-[300px] flex flex-col">
        {/* Close Button */}
        <button
          onClick={onClose}
          className="absolute top-4 right-4 p-1.5 rounded-full text-slate-400 hover:text-slate-600 hover:bg-slate-100 transition z-10"
        >
          <X className="w-5 h-5" />
        </button>
        ...
        {isLoading || !wordData ? (
          <div className="flex-1 flex flex-col items-center justify-center space-y-4">
            ...
            <p className="text-sm font-bold text-slate-500 animate-pulse">AI đang phân tích từ vựng...</p>
          </div>
        ) : (
          <>
            {/* Word Header */}
